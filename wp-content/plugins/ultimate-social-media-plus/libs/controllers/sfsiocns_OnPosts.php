...
function sfsi_plus_social_buttons_below($content)
{
	...
	if($sfsi_section8['sfsi_plus_recttwtr'] == 'yes')
	{
		...
		$icons.="<div class='sf_twiter' style='display: inline-block;vertical-align: middle;width: auto;'>".sfsi_twitterShare($permalink,$show_count).$count_html."</div>";
	}
...
function sfsi_twitterShare($permalink,$tweettext)
{
	...
	// $tweet_icon = SFSI_PLUGURL . 'images/visit_icons/'".$icons_language."'.svg';
...
function sfsi_plus_footer_script()
{
	...
	$isYoutubeFollowFeatureActive = (
		"yes" == $sfsi_section2['sfsi_plus_youtube_follow']) && 
		(
			isset($sfsi_section2['sfsi_plus_youtubeusernameorid']) && 
		...
		(
			(
				...
				!empty($sfsi_section2['sfsi_plus_ytube_user'])
			) || 
			( 
				"id" == $sfsi_section2['sfsi_plus_youtubeusernameorid'] && 
